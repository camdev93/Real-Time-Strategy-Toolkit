...
{
    // This will be a universal method that can be called to traverse the intended node
    // This method will allow for extraction of all children without children from the parent node in the argemuement parameter
    public static List<Node> TraverseGraphToExtractLowestLeafes(Node parentNode)
    {
        ...
        while (nodesToCheck.Count > 0)
        {
            ...
            else
            {
                ...
                {
                    nodesToCheck.Enqueue(child); //add all the nodes into the queue
                }
            }
        }
        return listToReturn;
